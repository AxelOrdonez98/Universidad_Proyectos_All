...
public class metodoBiseccion extends javax.swing.JFrame {
    ...
    private void btnCalcularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCalcularActionPerformed
       ...
       valorFuncionA=(x6*Math.pow(valorA, 6))+(x5*Math.pow(valorA, 5))+(x4*Math.pow(valorA, 4))+(x3*Math.pow(valorA, 3))+(x2*Math.pow(valorA, 2))+(x1*Math.pow(valorA, 1))+c;
        ...
        if(comparacionFunciones<0&&valorA!=valorB || valorB!=valorA){
        ...
        }while(errorRelativo>porError&&cont<50);
        txtAreaValores.setText(texto);
        lblIteraciones.setText(String.valueOf(cont+"       valor de la raiz: "+ valorPMedio));
        cont=0;
        texto="";
        }else {
